...
// ── University Hub ────────────────────────────────────────────────────────────
type UniversityTab = "chances" | "finder" | "prep" | "apply";

// ── Future Finder types ───────────────────────────────────────────────────────
type FutureFindTab = "uni" | "subjects" | "career";
...
const UNI_COUNTRIES = ["United Kingdom","United States","Canada","Australia","Netherlands","Germany","Singapore","India","Any"];
const STUDY_FIELDS  = ["Medicine","Engineering","Law","Economics / Finance","Computer Science","Architecture","Business","Natural Sciences","Psychology","Arts & Design","International Relations","Mathematics","Education","Journalism"];
const STUDY_BOARDS  = ["IB","A-Level","CBSE","ICSE","AP","SAT","IGCSE"];
...
function FutureFinderTab() {
  ...
  useEffect(() => { try { const a=localStorage.getItem("ledger-career-answers"); if(a) setAnswers(JSON.parse(a)); const o=localStorage.getItem("ledger-career-output"); if(o) setCarOutput(JSON.parse(o)); } catch {} }, []);
  ...
  function toggleCountry(c: string) { setCountries(p => p.includes(c)?p.filter(x=>x!==c):[...p,c]); }
  function toggleArr<T>(arr: T[], item: T, set: (v:T[])=>void) { set(arr.includes(item)?arr.filter(x=>x!==item):[...arr,item]); }
  ...
  const subjects    = spBoard==="IB"?IB_ALL:ALEVELS_LIST;
  const allAnswered = CAREER_QUESTIONS.every(q=>answers[q.id]);
  const currentQ    = CAREER_QUESTIONS[step];

  return (
    <div>
      <div style={{padding:"12px 44px",borderBottom:"1px solid var(--ink)",display:"flex",justifyContent:"flex-end"}}>
        <div style={{display:"flex",border:"1px solid var(--ink)"}}>
          <button style={ffTabStyle(tab==="uni")}      onClick={()=>setTab("uni")}>University Match</button>
          <button style={ffTabStyle(tab==="subjects")} onClick={()=>setTab("subjects")}>Subject Picker</button>
          <button style={{...ffTabStyle(tab==="career"),borderRight:"none"}} onClick={()=>setTab("career")}>Career Path</button>
        </div>
      </div>
      ...
      {tab==="uni"&&!uniResult&&(
        <main className="mob-p" style={{padding:"40px 44px 80px",maxWidth:640,margin:"0 auto"}}>
          ...
          <div style={{marginBottom:16}}><div className="mono" style={{color:"var(--ink-3)",marginBottom:6}}>Your board</div>
            <div style={{display:"flex",flexWrap:"wrap",gap:6}}>{STUDY_BOARDS.map(b=><button key={b} onClick={()=>setBoard(b)} style={{fontFamily:"var(--mono)",fontSize:10,padding:"5px 10px",border:`1px solid ${board===b?"var(--ink)":"var(--rule)"}`,background:board===b?"var(--ink)":"var(--paper)",color:board===b?"var(--paper)":"var(--ink)",cursor:"pointer"}}>{b}</button>)}</div>
          </div>
          <div style={{marginBottom:16}}><div className="mono" style={{color:"var(--ink-3)",marginBottom:6}}>Predicted / achieved grades</div>
            <input value={grade} onChange={e=>setGrade(e.target.value)} placeholder="e.g. IB 38/42, A-Level AAB, CBSE 94%…" style={{width:"100%",fontFamily:"var(--sans)",fontSize:13,border:"1px solid var(--ink)",background:"var(--paper)",padding:"10px 12px",color:"var(--ink)",boxSizing:"border-box"}} />
          </div>
          <div style={{marginBottom:16}}><div className="mono" style={{color:"var(--ink-3)",marginBottom:6}}>Field of study</div>
            <div style={{display:"flex",flexWrap:"wrap",gap:6}}>{STUDY_FIELDS.map(f=><button key={f} onClick={()=>setField(f)} style={{fontFamily:"var(--mono)",fontSize:10,padding:"5px 10px",border:`1px solid ${field===f?"var(--cinnabar-ink)":"var(--rule)"}`,background:field===f?"var(--cinnabar-ink)":"var(--paper)",color:field===f?"var(--paper)":"var(--ink)",cursor:"pointer"}}>{f}</button>)}</div>
          </div>
          <div style={{marginBottom:16}}><div className="mono" style={{color:"var(--ink-3)",marginBottom:6}}>Countries</div>
            <div style={{display:"flex",flexWrap:"wrap",gap:6}}>{UNI_COUNTRIES.map(c=><button key={c} onClick={()=>toggleCountry(c)} style={{fontFamily:"var(--mono)",fontSize:10,padding:"5px 10px",border:`1px solid ${countries.includes(c)?"var(--ink)":"var(--rule)"}`,background:countries.includes(c)?"var(--ink)":"var(--paper)",color:countries.includes(c)?"var(--paper)":"var(--ink)",cursor:"pointer"}}>{c}</button>)}</div>
          </div>
          <div style={{marginBottom:20}}><div className="mono" style={{color:"var(--ink-3)",marginBottom:6}}>Anything else? (optional)</div>
            <input value={extra} onChange={e=>setExtra(e.target.value)} placeholder="e.g. small campus, strong research, scholarship opportunities…" style={{width:"100%",fontFamily:"var(--sans)",fontSize:13,border:"1px solid var(--ink)",background:"var(--paper)",padding:"10px 12px",color:"var(--ink)",boxSizing:"border-box"}} />
          </div>
          {unError&&<div style={{color:"var(--cinnabar-ink)",fontFamily:"var(--sans)",fontSize:13,marginBottom:12}}>{unError}</div>}
          <button className="btn" onClick={generateUni} disabled={unLoading} style={{width:"100%",opacity:unLoading?0.5:1}}>{unLoading?"Finding your matches…":"Find my universities →"}</button>
      ...
      {tab==="uni"&&uniResult&&(
        <main className="mob-p" style={{padding:"40px 44px 80px",maxWidth:860,margin:"0 auto"}}>
          <div style={{display:"flex",justifyContent:"flex-end",marginBottom:20}}><button className="btn ghost" onClick={()=>setUniResult(null)}>Start over</button></div>
            ...
            {uniResult.unis.map((u,i)=>(
              <div key={i} style={{border:i===0?"2px solid var(--ink)":"1px solid var(--rule)",padding:"20px 24px"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}>
                  <div>{i===0&&<div className="mono cin" style={{marginBottom:4}}>Top match</div>}<div style={{fontFamily:"var(--sans)",fontSize:16,fontWeight:700}}>{u.name}</div><div className="mono" style={{fontSize:9,color:"var(--ink-3)",marginTop:2}}>{u.country}</div></div>
                  <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4}}><span style={{fontFamily:"var(--mono)",fontSize:10,padding:"3px 8px",border:`1px solid ${reachColor(u.reach)}`,color:reachColor(u.reach)}}>{reachLabel(u.reach)}</span><span className="mono" style={{fontSize:10,color:"var(--cinnabar-ink)"}}>{u.fitScore}/10 fit</span></div>
                </div>
                <div style={{fontFamily:"var(--sans)",fontSize:13,lineHeight:1.6,color:"var(--ink-2)",marginBottom:10}}>{u.why}</div>
                <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:10}}>{u.strengths.map((s,j)=><span key={j} style={{fontFamily:"var(--mono)",fontSize:9,padding:"3px 8px",border:"1px solid var(--rule)",color:"var(--ink-2)"}}>{s}</span>)}</div>
                <div style={{display:"flex",gap:12,flexWrap:"wrap"}}>
                  <div style={{padding:"8px 12px",background:"var(--paper-2)",border:"1px solid var(--rule)"}}><span className="mono" style={{fontSize:9,color:"var(--ink-3)"}}>REQUIREMENTS · </span><span style={{fontFamily:"var(--sans)",fontSize:12}}>{u.requirements}</span></div>
                  {u.applyBy&&<div style={{padding:"8px 12px",background:"var(--paper-2)",border:"1px solid var(--rule)"}}><span className="mono" style={{fontSize:9,color:"var(--ink-3)"}}>APPLY BY · </span><span style={{fontFamily:"var(--sans)",fontSize:12}}>{u.applyBy}</span></div>}
                </div>
              </div>
            ))}
          </div>
      ...
      {tab==="subjects"&&!spResult&&(
        <main className="mob-p" style={{padding:"40px 44px 80px",maxWidth:680,margin:"0 auto"}}>
          ...
          <div style={{marginBottom:20}}><div className="mono" style={{color:"var(--ink-3)",marginBottom:8}}>Your board</div>
            <div style={{display:"flex",gap:8}}>{(["IB","A-Level"] as const).map(b=><button key={b} onClick={()=>{setSpBoard(b);setInterests([]);}} style={{flex:1,padding:"12px",border:`2px solid ${spBoard===b?"var(--ink)":"var(--rule)"}`,background:spBoard===b?"var(--ink)":"var(--paper)",color:spBoard===b?"var(--paper)":"var(--ink)",fontFamily:"var(--sans)",fontSize:14,fontWeight:600,cursor:"pointer"}}>{b}</button>)}</div>
          </div>
          <div style={{marginBottom:20}}><div className="mono" style={{color:"var(--ink-3)",marginBottom:8}}>Subjects you like or are good at (pick 2–6)</div>
            <div style={{display:"flex",flexWrap:"wrap",gap:6}}>{subjects.map(s=><button key={s} onClick={()=>toggleArr(interests,s,setInterests)} style={{fontFamily:"var(--mono)",fontSize:10,padding:"5px 10px",border:`1px solid ${interests.includes(s)?"var(--ink)":"var(--rule)"}`,background:interests.includes(s)?"var(--ink)":"var(--paper)",color:interests.includes(s)?"var(--paper)":"var(--ink)",cursor:"pointer"}}>{s}</button>)}</div>
          </div>
          <div style={{marginBottom:20}}><div className="mono" style={{color:"var(--ink-3)",marginBottom:8}}>Career interests (pick 1–3)</div>
            <div style={{display:"flex",flexWrap:"wrap",gap:6}}>{CAREERS_LIST.map(c=><button key={c} onClick={()=>toggleArr(career,c,setCareer)} style={{fontFamily:"var(--mono)",fontSize:10,padding:"5px 10px",border:`1px solid ${career.includes(c)?"var(--cinnabar-ink)":"var(--rule)"}`,background:career.includes(c)?"var(--cinnabar-ink)":"var(--paper)",color:career.includes(c)?"var(--paper)":"var(--ink)",cursor:"pointer"}}>{c}</button>)}</div>
          </div>
          <div style={{marginBottom:20}}><div className="mono" style={{color:"var(--ink-3)",marginBottom:6}}>Anything else? (optional)</div>
            <input value={spExtra} onChange={e=>setSpExtra(e.target.value)} placeholder="e.g. I want to apply to UK universities…" style={{width:"100%",fontFamily:"var(--sans)",fontSize:13,border:"1px solid var(--ink)",background:"var(--paper)",padding:"10px 12px",color:"var(--ink)",boxSizing:"border-box"}} />
          </div>
          {spError&&<div style={{color:"var(--cinnabar-ink)",fontFamily:"var(--sans)",fontSize:13,marginBottom:12}}>{spError}</div>}
          <button className="btn" onClick={generateSubjects} disabled={spLoading} style={{width:"100%",opacity:spLoading?0.5:1}}>{spLoading?"Finding your perfect combination…":"Get subject recommendations →"}</button>
      ...
      {tab==="subjects"&&spResult&&(
        <main className="mob-p" style={{padding:"40px 44px 80px",maxWidth:860,margin:"0 auto"}}>
          <div style={{display:"flex",justifyContent:"flex-end",marginBottom:20}}><button className="btn ghost" onClick={()=>setSpResult(null)}>Start over</button></div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:18,marginBottom:28}}>
            {spResult.combos.map((c,i)=>(
              <div key={i} style={{border:i===0?"2px solid var(--ink)":"1px solid var(--rule)",padding:"20px 24px"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12}}>
                  <div>{i===0&&<div className="mono cin" style={{marginBottom:6}}>Best fit for you</div>}<div style={{display:"flex",flexWrap:"wrap",gap:6}}>{c.combo.map((s,j)=><span key={j} style={{fontFamily:"var(--sans)",fontSize:12,fontWeight:600,padding:"4px 10px",background:i===0?"var(--ink)":"var(--paper-2)",color:i===0?"var(--paper)":"var(--ink)",border:"1px solid var(--rule)"}}>{s}</span>)}</div></div>
                  <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4}}><span className="mono" style={{fontSize:10,color:diffColor(c.difficulty)}}>{c.difficulty.toUpperCase()}</span><span className="mono" style={{fontSize:11,color:"var(--cinnabar-ink)"}}>{c.score}/10</span></div>
                </div>
                <div style={{fontFamily:"var(--sans)",fontSize:13,lineHeight:1.6,color:"var(--ink-2)",marginBottom:10}}>{c.why}</div>
                <div style={{marginBottom:8}}><span className="mono" style={{fontSize:9,color:"var(--ink-3)"}}>OPENS DOORS TO · </span>{c.careerFit.map((cf,j)=><span key={j} style={{fontFamily:"var(--sans)",fontSize:11,color:"var(--ink-2)"}}>{cf}{j<c.careerFit.length-1?" · ":""}</span>)}</div>
                <div style={{padding:"8px 12px",background:"var(--paper-2)",border:"1px solid var(--rule)"}}><span className="mono" style={{fontSize:9,color:"var(--ink-3)"}}>UNI REQUIREMENTS · </span><span style={{fontFamily:"var(--sans)",fontSize:12,color:"var(--ink-2)"}}>{c.uniReqs}</span></div>
              </div>
            ))}
          </div>
      ...
      {tab==="career"&&!carOutput&&(
        <main className="mob-p" style={{padding:"40px 44px 80px",maxWidth:680,margin:"0 auto"}}>
          ...
          <div style={{marginBottom:16,padding:"10px 14px",background:"var(--paper-2)",border:"1px solid var(--rule)"}}>
            <div className="mono" style={{fontSize:9,color:"var(--ink-3)"}}>QUESTION {step+1} OF {CAREER_QUESTIONS.length}</div>
            <div style={{height:3,background:"var(--rule)",marginTop:8,overflow:"hidden"}}><div style={{height:"100%",width:"100%",background:"var(--cinnabar-ink)",transform:`scaleX(${(step+1)/CAREER_QUESTIONS.length})`,transformOrigin:"left",transition:"transform 300ms"}} /></div>
          </div>
          <div style={{border:"2px solid var(--ink)",padding:"24px 28px",marginBottom:20}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {currentQ.opts.map(opt=>(
                <button key={opt} onClick={()=>{setAnswers(a=>({...a,[currentQ.id]:opt}));if(step<CAREER_QUESTIONS.length-1)setStep(s=>s+1);}} style={{padding:"12px 16px",border:`1px solid ${answers[currentQ.id]===opt?"var(--ink)":"var(--rule)"}`,background:answers[currentQ.id]===opt?"var(--ink)":"var(--paper)",color:answers[currentQ.id]===opt?"var(--paper)":"var(--ink)",cursor:"pointer",textAlign:"left",fontFamily:"var(--sans)",fontSize:13}}>{opt}</button>
              ))}
            </div>
          </div>
          <div style={{display:"flex",gap:8,marginBottom:20}}>
            ...
            {step<CAREER_QUESTIONS.length-1
              ?<button onClick={()=>setStep(s=>Math.min(CAREER_QUESTIONS.length-1,s+1))} disabled={!answers[currentQ.id]} className="btn" style={{flex:1,opacity:answers[currentQ.id]?1:0.4}}>Next question →</button>
      ...
      {tab==="career"&&carOutput&&(
        <main className="mob-p" style={{padding:"40px 44px 80px",maxWidth:1280,margin:"0 auto"}}>
          <div style={{display:"flex",justifyContent:"flex-end",marginBottom:20}}><button className="btn ghost" onClick={()=>{setCarOutput(null);setAnswers({});setStep(0);}}>Retake quiz</button></div>
            ...
            <div>
              ...
              {carOutput.streams.map((s,i)=>(
                <div key={i} style={{border:"1px solid var(--ink)",borderBottom:i<carOutput.streams.length-1?"none":"1px solid var(--ink)",padding:"20px 18px"}}>
                  <div style={{display:"flex",alignItems:"baseline",gap:10}}><span className="mono cin">{String(i+1).padStart(2,"0")}</span><span style={{fontFamily:"var(--serif)",fontSize:22,fontWeight:600}}>{s.name}</span></div>
                  <div style={{margin:"8px 0 10px"}}><AIOutput text={s.why} /></div>
                  <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{s.roles.map((r,j)=><span key={j} style={{fontFamily:"var(--mono)",fontSize:9,padding:"3px 8px",border:"1px solid var(--rule)",color:"var(--ink-3)",textTransform:"uppercase"}}>{r}</span>)}</div>
                </div>
              ))}
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:24}}>
              <div><div className="mono cin" style={{marginBottom:10}}>Target colleges</div>
                <div style={{border:"1px solid var(--ink)"}}>{carOutput.colleges.map((c,i)=>(<div key={i} style={{padding:"12px 16px",borderBottom:i<carOutput.colleges.length-1?"1px solid var(--rule)":"none",display:"flex",gap:12,alignItems:"baseline"}}><span className="mono" style={{color:"var(--cinnabar-ink)",flexShrink:0}}>{String(i+1).padStart(2,"0")}</span><div><div style={{fontFamily:"var(--sans)",fontSize:13,fontWeight:600}}>{c.name}</div><div style={{fontFamily:"var(--sans)",fontSize:12,color:"var(--ink-2)"}}>{c.country} · {c.why}</div></div></div>))}</div>
              </div>
              <div><div className="mono cin" style={{marginBottom:10}}>Entrance exams</div>
                <div style={{border:"1px solid var(--ink)"}}>{carOutput.exams.map((e,i)=>(<div key={i} style={{padding:"12px 16px",borderBottom:i<carOutput.exams.length-1?"1px solid var(--rule)":"none"}}><div style={{fontFamily:"var(--sans)",fontSize:13,fontWeight:600}}>{e.name}</div><div style={{fontFamily:"var(--sans)",fontSize:12,color:"var(--ink-2)"}}>{e.desc}</div></div>))}</div>
              </div>
            </div>
          </div>
          <div style={{marginTop:40}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",borderTop:"1px solid var(--ink)",borderLeft:"1px solid var(--ink)"}}>
              {carOutput.roadmap.map((r,i)=>(<div key={i} style={{borderRight:"1px solid var(--ink)",borderBottom:"1px solid var(--ink)",padding:"18px 16px"}}><div className="mono cin" style={{marginBottom:8}}>{r.period}</div><ul style={{margin:0,paddingLeft:16,fontFamily:"var(--sans)",fontSize:12.5,lineHeight:1.7,color:"var(--ink-2)"}}>{r.milestones.map((m,j)=><li key={j}>{m}</li>)}</ul></div>))}
            </div>
          </div>
        </main>
      )}
...
function UniPrepTab() {
  ...
  const [result,      setResult]      = useState<UniPrepResult|null>(null);
  ...
  return (
    <div style={{maxWidth:860,margin:"0 auto",padding:"40px 24px 80px"}}>
      <div style={{marginBottom:32}}>
        ...
        <h1 style={{fontFamily:"var(--serif)",fontSize:28,fontStyle:"italic",fontWeight:700,color:"var(--ink)",margin:"10px 0 0",letterSpacing:"-0.02em"}}>University Prep</h1>
        <p style={{fontFamily:"var(--sans)",fontSize:14,color:"var(--ink-2)",marginTop:8,lineHeight:1.6}}>Get an honest readiness assessment and a month-by-month preparation roadmap for your target university.</p>
      </div>
      <form onSubmit={handleSubmit}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:0,border:"1px solid var(--ink)"}}>
          <div style={{padding:"16px 18px",borderRight:"1px solid var(--ink)",borderBottom:"1px solid var(--ink)"}}><label style={{fontFamily:"var(--mono)",fontSize:10,letterSpacing:"0.1em",color:"var(--ink-3)",display:"block",marginBottom:6}}>TARGET INSTITUTION *</label><input value={institution} onChange={e=>setInstitution(e.target.value)} placeholder="e.g. Imperial College London" style={{width:"100%",border:"none",background:"transparent",fontFamily:"var(--sans)",fontSize:14,color:"var(--ink)",outline:"none"}} required /></div>
          <div style={{padding:"16px 18px",borderBottom:"1px solid var(--ink)"}}><label style={{fontFamily:"var(--mono)",fontSize:10,letterSpacing:"0.1em",color:"var(--ink-3)",display:"block",marginBottom:6}}>COURSE *</label><input value={course} onChange={e=>setCourse(e.target.value)} placeholder="e.g. MEng Electrical Engineering" style={{width:"100%",border:"none",background:"transparent",fontFamily:"var(--sans)",fontSize:14,color:"var(--ink)",outline:"none"}} required /></div>
          <div style={{padding:"16px 18px",borderRight:"1px solid var(--ink)",borderBottom:"1px solid var(--ink)"}}><label style={{fontFamily:"var(--mono)",fontSize:10,letterSpacing:"0.1em",color:"var(--ink-3)",display:"block",marginBottom:6}}>APPLICATION CYCLE</label><input value={cycle} onChange={e=>setCycle(e.target.value)} placeholder="e.g. 2026 entry" style={{width:"100%",border:"none",background:"transparent",fontFamily:"var(--sans)",fontSize:14,color:"var(--ink)",outline:"none"}} /></div>
          <div style={{padding:"16px 18px",borderBottom:"1px solid var(--ink)"}}><label style={{fontFamily:"var(--mono)",fontSize:10,letterSpacing:"0.1em",color:"var(--ink-3)",display:"block",marginBottom:6}}>PREDICTED / CURRENT GRADES</label><input value={grades} onChange={e=>setGrades(e.target.value)} placeholder="e.g. A-Level predicted A*A*A" style={{width:"100%",border:"none",background:"transparent",fontFamily:"var(--sans)",fontSize:14,color:"var(--ink)",outline:"none"}} /></div>
        </div>
        <div style={{border:"1px solid var(--ink)",borderTop:"none",padding:"16px 18px"}}><label style={{fontFamily:"var(--mono)",fontSize:10,letterSpacing:"0.1em",color:"var(--ink-3)",display:"block",marginBottom:6}}>YOUR PROFILE — ECs, competitions, work experience, motivations</label><textarea value={profile} onChange={e=>setProfile(e.target.value)} rows={4} placeholder="e.g. Robotic club president, UKMT Intermediate Gold, internship at local tech firm…" style={{width:"100%",border:"none",background:"transparent",fontFamily:"var(--sans)",fontSize:14,color:"var(--ink)",outline:"none",resize:"vertical",lineHeight:1.6}} /></div>
        <button type="submit" disabled={loading||!institution.trim()||!course.trim()} className="btn" style={{marginTop:14,cursor:"pointer",opacity:loading?0.6:1}}>{loading?"Generating…":"Assess My Readiness"}</button>
      </form>
      {loading&&<div style={{marginTop:32}}><AIThinking /></div>}
      {error&&<div style={{marginTop:20,fontFamily:"var(--sans)",fontSize:13,color:"var(--cinnabar-ink)"}}>{error}</div>}
      {result&&(
        <div style={{marginTop:40}}>
          <div style={{border:"1px solid var(--ink)",padding:"20px 22px",background:"var(--paper-2)"}}><div style={{fontFamily:"var(--mono)",fontSize:10,letterSpacing:"0.12em",color:"var(--cinnabar-ink)",marginBottom:8}}>{result.institution} · {result.course} · {result.applicationCycle}</div><p style={{fontFamily:"var(--sans)",fontSize:14,color:"var(--ink)",margin:0,lineHeight:1.7}}>{result.profileAssessment}</p></div>
          ...
          <div style={{border:"1px solid var(--cinnabar-ink)",borderTop:"none",padding:"16px 22px"}}><div style={{fontFamily:"var(--mono)",fontSize:10,letterSpacing:"0.12em",color:"var(--ink-3)",marginBottom:6}}>INSIDER ADVICE</div><p style={{fontFamily:"var(--serif)",fontStyle:"italic",fontSize:15,color:"var(--ink)",margin:0,lineHeight:1.7}}>{result.advice}</p></div>
          ...
          {tab==="essays"&&(<div style={{border:"1px solid var(--ink)",borderTop:"none",padding:"20px 22px"}}><div style={{fontFamily:"var(--mono)",fontSize:10,color:"var(--ink-3)",letterSpacing:"0.1em",marginBottom:14}}>PERSONAL STATEMENT ANGLES</div>{(result.essayTopics??[]).map((t,i)=>(<div key={i} style={{border:"1px solid var(--cinnabar)",padding:"0 16px",marginBottom:16}}><p style={{fontFamily:"var(--sans)",fontSize:14,color:"var(--ink)",margin:0,lineHeight:1.65}}>{t}</p></div>))}</div>)}
        </div>
      )}
    </div>
  );
...
function ApplicationsTab() {
  ...
  const [profile,     setProfile]     = useState("");
  const [grades,      setGrades]      = useState("");
  const [result,      setResult]      = useState<AppPlanResult|null>(null);
  const [loading,     setLoading]     = useState(false);
  const [error,       setError]       = useState("");
  ...
  return (
    <div style={{maxWidth:860,margin:"0 auto",padding:"40px 24px 80px"}}>
      <div style={{marginBottom:32}}>
        <span style={{fontFamily:"var(--mono)",fontSize:9,letterSpacing:"0.16em",textTransform:"uppercase",color:"var(--cinnabar-ink)",border:"1px solid var(--cinnabar-ink)",padding:"2px 6px"}}>FUTURE</span>
        <h1 style={{fontFamily:"var(--serif)",fontSize:28,fontStyle:"italic",fontWeight:700,color:"var(--ink)",margin:"10px 0 0",letterSpacing:"-0.02em"}}>Application Planner</h1>
        <p style={{fontFamily:"var(--sans)",fontSize:14,color:"var(--ink-2)",marginTop:8,lineHeight:1.6}}>Generate a structured, task-by-task application plan for any university.</p>
      </div>
      <form onSubmit={handleSubmit}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:0,border:"1px solid var(--ink)"}}>
          <div style={{padding:"16px 18px",borderRight:"1px solid var(--ink)",borderBottom:"1px solid var(--ink)"}}><label style={{fontFamily:"var(--mono)",fontSize:10,letterSpacing:"0.1em",color:"var(--ink-3)",display:"block",marginBottom:6}}>INSTITUTION *</label><input value={institution} onChange={e=>setInstitution(e.target.value)} placeholder="e.g. University of Edinburgh" style={{width:"100%",border:"none",background:"transparent",fontFamily:"var(--sans)",fontSize:14,color:"var(--ink)",outline:"none"}} required /></div>
          <div style={{padding:"16px 18px",borderBottom:"1px solid var(--ink)"}}><label style={{fontFamily:"var(--mono)",fontSize:10,letterSpacing:"0.1em",color:"var(--ink-3)",display:"block",marginBottom:6}}>COURSE *</label><input value={course} onChange={e=>setCourse(e.target.value)} placeholder="e.g. BSc Computer Science" style={{width:"100%",border:"none",background:"transparent",fontFamily:"var(--sans)",fontSize:14,color:"var(--ink)",outline:"none"}} required /></div>
          <div style={{padding:"16px 18px",borderRight:"1px solid var(--ink)",borderBottom:"1px solid var(--ink)"}}><label style={{fontFamily:"var(--mono)",fontSize:10,letterSpacing:"0.1em",color:"var(--ink-3)",display:"block",marginBottom:6}}>DEADLINE</label><input value={deadline} onChange={e=>setDeadline(e.target.value)} placeholder="e.g. Jan 15, 2026" style={{width:"100%",border:"none",background:"transparent",fontFamily:"var(--sans)",fontSize:14,color:"var(--ink)",outline:"none"}} /></div>
          <div style={{padding:"16px 18px",borderBottom:"1px solid var(--ink)"}}><label style={{fontFamily:"var(--mono)",fontSize:10,letterSpacing:"0.1em",color:"var(--ink-3)",display:"block",marginBottom:6}}>CURRENT GRADES</label><input value={grades} onChange={e=>setGrades(e.target.value)} placeholder="e.g. IB predicted 40/45, Maths HL 7" style={{width:"100%",border:"none",background:"transparent",fontFamily:"var(--sans)",fontSize:14,color:"var(--ink)",outline:"none"}} /></div>
        </div>
        <div style={{border:"1px solid var(--ink)",borderTop:"none",padding:"16px 18px"}}><label style={{fontFamily:"var(--mono)",fontSize:10,letterSpacing:"0.1em",color:"var(--ink-3)",display:"block",marginBottom:6}}>YOUR PROFILE — activities, achievements, interests, why this course</label><textarea value={profile} onChange={e=>setProfile(e.target.value)} rows={4} placeholder="e.g. Physics Olympiad bronze, founded school coding club…" style={{width:"100%",border:"none",background:"transparent",fontFamily:"var(--sans)",fontSize:14,color:"var(--ink)",outline:"none",resize:"vertical",lineHeight:1.6}} /></div>
        <button type="submit" disabled={loading||!institution.trim()||!course.trim()} className="btn" style={{marginTop:14,cursor:"pointer",opacity:loading?0.6:1}}>{loading?"Generating…":"Generate Application Plan"}</button>
      </form>
      {loading&&<div style={{marginTop:32}}><AIThinking /></div>}
      {error&&<div style={{marginTop:20,fontFamily:"var(--sans)",fontSize:13,color:"var(--cinnabar-ink)"}}>{error}</div>}
      {result&&(
        <div style={{marginTop:40}}>
          ...
          {tab==="tasks"&&(<div style={{border:"1px solid var(--ink)",borderTop:"none"}}>{(result.tasks??[]).map((t,i)=>(<div key={i} style={{display:"grid",gridTemplateColumns:"1fr auto auto",gap:16,alignItems:"center",padding:"14px 18px",borderBottom:i<result.tasks.length-1?"1px solid var(--rule)":"none"}}><span style={{fontFamily:"var(--sans)",fontSize:14,color:"var(--ink)"}}>{t.task}</span><span style={{fontFamily:"var(--mono)",fontSize:10,color:"var(--ink-3)",whiteSpace:"nowrap"}}>{t.due}</span><span style={{fontFamily:"var(--mono)",fontSize:9,letterSpacing:"0.1em",textTransform:"uppercase",color:APP_PRIORITY_COLOR[t.priority]??"var(--ink-3)",border:`1px solid ${APP_PRIORITY_COLOR[t.priority]??"var(--rule)"}`,padding:"2px 6px",whiteSpace:"nowrap"}}>{t.priority}</span></div>))}</div>)}
          {tab==="essays"&&(<div style={{border:"1px solid var(--ink)",borderTop:"none",padding:"20px 22px"}}><div style={{fontFamily:"var(--mono)",fontSize:10,color:"var(--ink-3)",letterSpacing:"0.1em",marginBottom:14}}>ESSAY PROMPTS TO PREPARE FOR</div>{(result.essayPrompts??[]).map((p,i)=>(<div key={i} style={{border:"1px solid var(--cinnabar)",padding:"0 16px",marginBottom:16}}><p style={{fontFamily:"var(--sans)",fontSize:14,color:"var(--ink)",margin:0,lineHeight:1.65}}>{p}</p></div>))}</div>)}
          {tab==="timeline"&&(<div style={{border:"1px solid var(--ink)",borderTop:"none"}}>{(result.timeline??[]).map((w,i)=>(<div key={i} style={{display:"grid",gridTemplateColumns:"60px 1fr",gap:0,borderBottom:i<result.timeline.length-1?"1px solid var(--rule)":"none"}}><div style={{padding:"14px 0 14px 18px",borderRight:"1px solid var(--rule)",fontFamily:"var(--mono)",fontSize:11,color:"var(--cinnabar-ink)",display:"flex",alignItems:"center"}}>W{w.week}</div><div style={{padding:"14px 18px",fontFamily:"var(--sans)",fontSize:14,color:"var(--ink)"}}>{w.focus}</div></div>))}</div>)}
        </div>
      )}
    </div>
  );
...
export default function AdmissionsPage() {
  const [universityTab, setUniversityTab] = useState<UniversityTab>("chances");
  ...
  if (universityTab==="finder") return <div>{tabHeader}<FutureFinderTab /></div>;
  if (universityTab==="prep")   return <div>{tabHeader}<UniPrepTab /></div>;
  if (universityTab==="apply")  return <div>{tabHeader}<ApplicationsTab /></div>;
  ...
  if (phase === "profile") return (
    <TierGate requires="pro">
      <div>
        ...
        <main className="mob-p" style={{ padding:"40px 44px 80px", maxWidth:680, margin:"0 auto" }}>
          <div style={{ height:3, background:"var(--paper-2)", border:"1px solid var(--rule)", marginBottom:40, overflow:"hidden" }}>
            <div style={{ height:"100%", width:"100%", background:"var(--cinnabar)", transform:`scaleX(${(step+1)/2})`, transformOrigin:"left", transition:"transform 400ms" }} />
          </div>
              ...
              <div style={{ border:"1px solid var(--ink)", padding:"16px", marginBottom:20 }}>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr auto auto", gap:8, alignItems:"center" }}>
                  <div style={{ display:"flex", gap:4 }}>
                    {([["national","National"],["regional","Regional"],["local","Local"]] as [ECTier,string][]).map(([v,l]) => (
                      <button key={v} onClick={()=>setEcTier(v)} style={{ fontFamily:"var(--mono)", fontSize:10, padding:"6px 10px", border:`1px solid ${ecTier===v?"var(--ink)":"var(--rule)"}`, background:ecTier===v?"var(--ink)":"var(--paper)", color:ecTier===v?"var(--paper)":"var(--ink)", cursor:"pointer" }}>{l}</button>
                    ))}
                  </div>
