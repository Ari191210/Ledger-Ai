...
type Subject = { id: string; name: string; completion: number; examDate: string };
...
function debtScore(subjects: Subject[]) {
...
export default function DebtMeterPage() {
  ...
  function save(s: Subject[]) {
    setSubjects(s);
    try { localStorage.setItem("ledger-debt-subjects", JSON.stringify(s)); } catch {}
  }

  function update(id: string, field: keyof Subject, value: string | number) {
  ...
  return (
    <div>
      <header className="mob-hp" style={{ padding: "24px 44px", borderBottom: "1px solid var(--ink)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div className="mono" style={{ color: "var(--ink-3)" }}>Cognitive Debt Meter · α</div>
        <div className="mono" style={{ color: aptColor }}>{apt} debt level</div>
      </header>
      ...
      <main className="mob-p" style={{ padding: "40px 44px 80px", maxWidth: 960, margin: "0 auto" }}>
              ...
              <div className="mob-col" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                ...
                <div>
                  ...
                  <input type="date" value={s.examDate}
                    ...
                    style={{ width: "100%", fontFamily: "var(--mono)", fontSize: 12, border: "1px solid var(--rule)", background: "var(--paper)", padding: "6px 8px", color: "var(--ink)" }} />
                </div>
              </div>
              {s.examDate && (
                <div className="mono" style={{ fontSize: 9, color: "var(--ink-3)", marginTop: 8 }}>
                  {daysUntil(s.examDate)} days until exam · {Math.ceil((100 - s.completion) / Math.max(1, daysUntil(s.examDate) / 7))} chapters/week needed
                </div>
              )}
            </div>
          ))}
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 48 }}>
          ...
          <button className="btn" onClick={addSubject} disabled={!newName.trim()} style={{ opacity: newName.trim() ? 1 : 0.4 }}>Add</button>
        </div>
