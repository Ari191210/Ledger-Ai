    ...
    <div>
      ...
      <div style={{ display: "flex", gap: 0, border: "1px solid var(--ink)", marginBottom: 32, overflow: "hidden" }}>
        {(["CBSE", "JEE", "NEET", "IB", "IGCSE"] as HeatBoard[]).map((b, i, arr) => (
          <button key={b} onClick={() => setBoard(b)} style={{ flex: 1, padding: "10px 0", border: "none", borderRight: i < arr.length - 1 ? "1px solid var(--rule)" : "none", background: board === b ? "var(--ink)" : "var(--paper)", color: board === b ? "var(--paper)" : "var(--ink-2)", fontFamily: "var(--mono)", fontSize: 11, letterSpacing: "0.06em", cursor: "pointer", transition: "all 150ms" }}>{b}</button>
        ))}
      </div>
