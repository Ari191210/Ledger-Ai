...
- [x] Fix: Peer Heatmap — remove the "Updated hourly" text from the header and replace the "Aggregated from weak-topic data across Ledger users" description with "Sample data · Representative distribution across boards" since the data is hardcoded, not live
- [x] Fix: GPA Simulator — the page header reads "Score Needed Calculator" but the dashboard card says "GPA Simulator" — rename the h1/header in app/tools/gpa-sim/page.tsx to "GPA Simulator" to match
...
- [x] Feature: Past Papers AI explain — in app/tools/papers/page.tsx, after a user marks a question wrong, add an "Explain this answer" button that calls callAI with tool "papers_explain" passing the question and correct answer, and shows the explanation inline; also add case "papers_explain" to app/api/ai/route.ts
