...
export const TOOLS_REGISTRY: ToolEntry[] = [
  ...
  // Distinct from post-exam's own DNA tab (`/tools/dna` → `/tools/post-exam
  // ?tab=dna`, unlinked-not-deleted, next.config.mjs) — that tab reads a
  // local mistake log; this is the real `occurrences`/`patterns` (007, M11)
  // analytics view, one level deeper. Both stay reachable; neither redirects
  // to the other, since neither has fully absorbed the other's surface yet.
  { slug: "mistake-dna",        title: "Mistake DNA",            subtitle: "Your real mistake record, broken down.", cat: "PRACTISE", status: "core",         tier: "Free", blurb: "Every logged mistake, broken down by type, subject and topic — real counts from your own record, never a placeholder.", keywords: ["mistake","dna","pattern","conceptual","calculation","careless","recall","heatmap","trigger","recovery","fingerprint"] },
