# CONSOLE — The StudyLedger Design Constitution, v3
...
same pixels — aurora-and-glass vs editorial-paper vs tonal-glass — and the product landed
...
3. **A console as in cabinet** — engineered, housed, physical. Teenage Engineering.
...
7. **Never lie.** No fabricated trend, forecast, or encouragement. An honest empty state
   beats a fake number, always. (Inherited unchanged from the previous constitution — the
...
Glassmorphism · blurred orbs · aurora/animated backgrounds · purple→blue gradients ·
gradient text · glow as emphasis · custom cursors · cursor trails · typing effects ·
AI avatars · robot/sparkle iconography · confetti · mascots · badges · XP · levels ·
newspaper column layouts · article typography · rounded-card grids where every card is
identical · drop shadows as elevation · "unlock your potential"-class copy · the words
"AI-powered", "supercharge", "revolutionise" · tiny uppercase tracked eyebrows above every
section · testimonial carousels.
...
principle, it is wrong — however good it looks.
...
> 4. **With the wordmark removed, is it still recognisably StudyLedger — and recognisably
...
glyph (▲/▼) so colour is never the sole carrier — accessibility is not negotiable.
...
same signal, same semantics. It ships in Phase 3, not before — get one housing right first.

### 2.3 Typography Philosophy

**Two voices. Never three.** (Ratified 2026-08-04.)

**Voice 1 — Interface.** A neutral, engineered grotesk. Carries every word a student reads.

| Rank | Face | Status |
|---|---|---|
| Preferred | **ABC Diatype** (Dinamo) | Commercial licence required |
| Alternative | **Söhne** (Klim) | Commercial licence required |
| Fallback | **IBM Plex Sans** | Open (SIL OFL) — ships today, no cost |

**Voice 2 — Instrument. `IBM Plex Mono`, and it is part of the product identity.**

Not a utility font used grudgingly for code. It is the voice of the machine, and it owns
every figure in the product:

> Ledger Score · statistics · streaks · percentages · timers · rankings · analytics ·
> IDs · timestamps · technical UI · units · deltas · dates
...
later is a **one-line change** — which is precisely why the token layer ships in Phase 1
before any surface is converted. Do not let a licence purchase block Phase 0.

**Explicitly rejected, permanently:** Inter · Geist · Space Grotesk · Manrope · Outfit ·
DM Sans · Poppins · Roboto · Plus Jakarta Sans. Not because they are bad faces — because
...
**The ramp — six sizes, no exceptions:**

| Step | Use |
|---|---|
| `display` | The Ledger Score. One per screen, maximum. |
| `figure` | Secondary numerals — sector values, deltas. |
...
history was that *every* size was invented on the spot — that, not any colour choice, is
...
pill. Nothing is ever `16px+` — that is app-store softness and it reads as toy.

**Spacing is a 4px grid, expressed in six steps.** Density is a feature: this is an
instrument panel, not a landing page. Generous whitespace is *not* the goal — *legible
...
label controls; they never decorate. If a control can be a word, it is a word — words are
...
2. **Press has travel.** Controls compress `1–2px` and darken one graphite step. They
...
4. **The keyboard is a first-class citizen.** `⌘K` is not a feature — it is the primary
   navigation. Every action reachable in ≤2 keystrokes.
...
6. **No confirmation dialogs.** Perform the action, show an undo. Undo is always available
   for 8 seconds.
7. **Destructive actions require intent, not confirmation** — hold-to-delete, not
   "are you sure?".
...
| **Roll** | Any numeral that changes | Odometer — digits roll, they never fade |
...
roll and fill — the one place we let the student watch). Anything needing a fourth
...
A student who sees 46 links thinks "which one?" — an anxiety event, at the exact moment we
...
nav — settings live behind the account chip.

### 5.2 Command is the navigation
`⌘K` (and a persistent search affordance on touch) is how a student reaches anything.
They type what they want to *do* — "essay", "past paper", "why did I drop" — and the
...
|---|---|---|---|
| **Arrival** | Opening the app | *"It knows me."* | Recognition. Their name, their exam, their state — instantly, no loading |
...
shows the honest figure and one recovery action — never a verdict.
...
RECORD     Trajectory → sectors → close history
```

**NOW is the default and the product's face.** It must be readable in one glance, fit one
viewport at 1280×800 without scrolling for its primary answer, and never require a click
...
**NOW** — like picking up a device that's already on and already knows you. Score dominant,
one action, nothing competing. Silent, still, ready.

**WORK** — like the room disappearing. Chrome recedes, the task fills the frame. Input
...
**RECORD** — like reading an instrument log. Dense, factual, trustworthy. The one place
where more information is better. Never celebratory, never apologetic.

**ONBOARDING** — like a device's first boot. Three questions maximum (grade, board, target
...
**EMPTY STATES** — every empty state is an invitation with exactly one button. "Your record
opens at your first close. [Log a session]". Never an illustration. Never an apology.

**LOADING** — optimistic UI, so mostly it doesn't exist. Where genuinely unavoidable: the
final layout, at rest, with figures dashed. Never skeleton shimmer.

**SUCCESS** — the Score rolls. That's the celebration. No toast, no confetti, no modal. The
number moving *is* the reward, and it's honest.

**AI INTERACTIONS** — the student never sees a model, a token, a "thinking" spinner, or the
...
- The aurora system (already down to 5 references — finish it)
...
NOW · WORK · RECORD · the chrome and Score indicator · the component set (~15) · the token
layer · onboarding · every empty state · the command bar as primary navigation.
...
language is wrong and we fix it here — not after 46 pages are converted.

### Phase 1 — The foundation (1–2 weeks)
...
Chrome, the three-surface model, `⌘K` as primary navigation, persistent Score. Tools become
verbs. **Gate:** a student can reach any capability in ≤2 keystrokes and never sees a
46-item list.

### Phase 3 — WORK (2–3 weeks)
...
rationale. When a rule here is broken, the rule is amended or the code is fixed — never
both quietly.

- 2026-08-04 — Console ratified. Supersedes PRODUCT_CONSTITUTION.md, PRODUCT.md, DESIGN.md
...
- 2026-08-04 — Founder decisions, same session:
  · **Typography fixed.** Interface: ABC Diatype (preferred) → Söhne (alt) → IBM Plex Sans
  ...
  · **Signal fixed as Electric Lime**, with the signal-vs-brand distinction made explicit
    and enumerated.
  · **Influence rule added (§1.6).** Extract principles from the references; never imitate
    their appearance. The decision test was rewritten because its original wording
    ("would Teenage Engineering ship this") contradicted this rule — it now tests
